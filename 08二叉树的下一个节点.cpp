...
BinaryTreeNode* GetNext(BinaryTreeNode* pNode) {
    if (pNode == nullptr) {
        ...
        if (!pNode->m_pRight || !pNode->m_pLeft) {
            ...
            BinaryTreeNode* result = pNode->m_pRight;
            while (result->m_pLeft) {
        ...
        return result;
    } else if (!pNode->m_pRight && pNode == pNode->m_pParent->m_pLeft) {
        return pNode->m_pParent;
    } else {
        BinaryTreeNode* result = pNode->m_pParent;
        while (true) {
            if (!result->m_pParent) {
                return nullptr;
            } else if (result == result->m_pParent->m_pLeft) {
                return result->m_pParent;
            } else {
